...
testthat::test_that(
  "Tests on clip and mask BOA on extent", {
    ...
    testthat::expect_equal(mean(exp_stars[[1]][,,3], na.rm=TRUE), 705.5385, tolerance = 1e-03)
    testthat::expect_equal(sum(is.na(exp_stars[[1]][,,3])), 1493524, tolerance = 1e-03)
    rm(exp_stars)
...
testthat::test_that(
  "Tests on clip TOA on extent, reproject and resize and save as ENVI", {
    ...
    testthat::expect_equal(mean(exp_stars[[1]][,,3], na.rm=TRUE), 884.8637, tolerance = 1e-03)
