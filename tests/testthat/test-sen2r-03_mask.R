...
testthat::test_that(
  "Tests on base mask on BOA", {
    ...
    testthat::expect_true(round(mean(exp_stars[[1]][,,1], na.rm=TRUE)) %in% c(318, 337))
    testthat::expect_true(sum(is.na(exp_stars[[1]][,,1])) %in% c(1503871, 1430023))
    rm(exp_stars)
    exp_stars <- stars::read_stars(exp_outpath_5[2:3])
    exp_stars <- stars::read_stars(exp_outpath_5[2:3])
    testthat::expect_true(round(mean(exp_stars[[1]], na.rm=TRUE)) %in% c(1453, 1392))
    testthat::expect_true(round(mean(exp_stars[[2]], na.rm=TRUE)) %in% c(148, 117))
    testthat::expect_true(sum(is.na(exp_stars[[1]])) %in% c(1503871, 1430023))
    testthat::expect_true(sum(is.na(exp_stars[[2]])) %in% c(1493524, 1417518))
    rm(exp_stars)
