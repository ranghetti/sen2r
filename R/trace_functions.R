...
#' @param outfilenames Vector of the files intended to be created by `fun`
#'  (for now, providing it is mandatory).
#' @param funname The name of the function to be run (in [start_trace]) or
...
start_trace <- function(outfilenames, funname) {
  ...
  writeLines(outfilenames, tracename)
  # return its path
  tracename
...
#' @name trace_function
#' @rdname trace_function
trace_function <- function(fun, outfilenames, ...) {
  
  # Start tracing
  funname <- deparse(substitute(fun))
  tracename <- start_trace(outfilenames, funname)
...
#' @name clean_traces
#' @rdname trace_function
clean_traces <- function(funname=NA) {
  ...
  # if required, filter on funname
  if (!is.na(funname)) {
    tracenames_df <- tracenames_df[tracenames_df$fun==funname,]
  }
  # if empty, exit
  ...
  outfilenames_df <- data.frame(
    "name" = unlist(outfilenames),
    "tracename" = rep(tracenames_df$name, sapply(outfilenames, length)),
    stringsAsFactors = FALSE
  )
  outfilenames_df$time <- file.mtime(outfilenames_df$name)
  outfilenames_df$tracetime <- tracenames_df[
    match(outfilenames_df$tracename, tracenames_df$name),
    "time"
  ]
  
  # delete files if they were created after each tracetime
  for (i in seq_len(nrow(outfilenames_df))) {
    if (file.exists(outfilenames_df[i,"name"])) {
      if (outfilenames_df[i,"time"] > outfilenames_df[i,"tracetime"]) {
        unlink(outfilenames_df[i,"name"])
      }
    }
  }
